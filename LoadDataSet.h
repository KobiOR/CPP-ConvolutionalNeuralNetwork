...
void parse_mnist_labels(const std::string label_file, std::vector<label_t> *labels) {
    ...
    for (size_t i = 0; i < num_items; i++) {
        ...
        labels->push_back((unsigned long &&) (label_t) label);
    }
}
...
void parse_mnist_image(std::ifstream& ifs,const mnist_header& header,double scale_min, double scale_max,int x_padding,int y_padding,vec_t& dst) {
    ...
    dst.resize((unsigned long) (width * height), scale_min);
...
void parse_mnist_images(const std::string image_file,std::vector<vec_t> *images,double scale_min = -1.0,double scale_max = 1.0,int x_padding = 2,int y_padding = 2) {
